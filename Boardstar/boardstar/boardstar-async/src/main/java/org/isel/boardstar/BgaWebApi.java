...
public class BgaWebApi {
    ...
    public BgaWebApi(AsyncRequest req, Gson gson) {
        this.req = req;
        this.gson = gson;
    }
    ...
    public CompletableFuture<List<CategoryDto>> getCategories() {
        ...
        return req.getBody(path).thenApply(body -> {
            GetCategoriesDto dto = gson.fromJson(body, GetCategoriesDto.class);
            return dto.getCategories();
        });
    }

    public CompletableFuture<List<GameDto>> searchByCategories(int page, String... categoriesIDs) {
    ...
    public CompletableFuture<List<GameDto>> searchByArtist(int page, String artist) {
        page = (page - 1) * PAGE_LIMIT;
